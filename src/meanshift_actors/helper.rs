...
use crate::meanshift_base::{mean_shift_single, DistanceMeasure, LibData, RefArray};
use std::sync::Arc;
...
impl<A: LibData> Handler<MeanShiftHelperWorkMessage<A>> for MeanShiftHelper<A> {
    ...
    fn handle(
        &mut self,
        msg: MeanShiftHelperWorkMessage<A>,
        ctx: &mut Self::Context,
    ) -> Self::Result {
        let (mean, points_within_len, iterations) =
            self.mean_shift_single(msg.start_center, self.bandwidth);
        msg.source
            .do_send(MeanShiftHelperResponse {
